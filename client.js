...
jQuery(function () {
    ...
    .on("click", "#btn-delete-card", function () {
        socket.send("delete\\card\\"+modal_cardnum);
    });
<<<<<<< HEAD
    $( '#listform' ).sortable({
        items: $('.mylist')
...
$(document).ready(function () {
    ...
    .on('click', 'span', function () {
        modal_cardnum = $(this).parent().parent().attr("data-cardindex");
        socket.send("load\\descript\\"+modal_cardnum);
        $('#myModal').modal('show');
    })
    ...
    $("#adjust").on('click', '.btn-adj', function () {
        //수정은 햇는데 좀 보자
        var value= $(".title").val();
        $('[data-cardindex='+modal_cardnum+']').find("span").val(value);
     })

    $("#btn-add-list").on("click", function () {
        var temp = $("#newlist").removeClass('d-none');
        temp.appendTo("#listform");
>>>>>>> 9175e5099690e547a462656e91b9b6227fa5bcce
    });
});
